        ...
        <div className="flex items-center gap-4 sm:gap-6 min-w-0">
          ...
          <div className="min-w-0">
            <h1 className="text-xl sm:text-2xl font-bold text-[#172033] truncate">
              {profile?.investorName || user?.company_name || user?.name || "Investor Name"}
            </h1>
            <p className="text-sm font-semibold text-[#8E1B2E] mt-1">
              {profile?.designation ? `${profile.designation} • ` : ""}
              {profile?.investingAs ? `${profile.investingAs.toUpperCase()} INVESTOR` : "INVESTOR"}
            </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
        ...
        <div className="lg:col-span-2 space-y-6 sm:space-y-8">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5 sm:p-8 shadow-sm">
            ...
            <SectionHeader>Investment Thesis / About</SectionHeader>
            <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-line">
              {profile?.about || "No investment thesis specified."}
            </p>
          </div>

          {/* Investment Stages */}
          <div className="bg-white rounded-2xl border border-gray-100 p-5 sm:p-8 shadow-sm">
            <SectionHeader>Investment Stages</SectionHeader>
            <div>
              {profile?.investmentStages?.length ? (
                profile.investmentStages.map((stage) => <Tag key={stage}>{stage}</Tag>)
              ) : (
                <p className="text-xs text-gray-400">All investment stages.</p>
              )}
            </div>
          </div>
        ...
        <div className="space-y-6 sm:space-y-8">
          <div className="bg-white rounded-2xl border border-gray-100 p-5 sm:p-8 shadow-sm">
            <SectionHeader>Ticket Size</SectionHeader>
            <p className="text-xl font-extrabold text-[#8E1B2E]">
              {profile?.checkSize || "Not specified"}
            </p>
          </div>
